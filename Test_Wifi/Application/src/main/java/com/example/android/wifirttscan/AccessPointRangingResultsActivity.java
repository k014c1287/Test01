...
public class AccessPointRangingResultsActivity extends AppCompatActivity {
    ...
    private static final int SAMPLE_SIZE_DEFAULT = 10;
    ...
    private void startRangingRequest() {       //APまでの距離測定をリクエスト
    ...
    private class RttRangingResultCallback extends RangingResultCallback {
        ...
        @Override
        public void onRangingResults(@NonNull List<RangingResult> list) {   //距離等の情報の獲得
            ...
            if (list.size() == 1) {
                ...
                if (mMAC.equals(rangingResult.getMacAddress().toString())) {
                    ...
                    if (rangingResult.getStatus() == RangingResult.STATUS_SUCCESS) {
                        ...
                        mRangeTextView.setText((rangingResult.getDistanceMm() / 1000f) + "");   //距離がミリメートル単位でint型が返ってくるのでメートル単位に直してfloatで処理
                        AP_Range = (rangingResult.getDistanceMm() / 1000f);
                        addDistanceToHistory(rangingResult.getDistanceMm());
                        mRangeMeanTextView.setText((getDistanceMean() / 1000f) + "");   //meanが何を表すかは不明

                        mRangeSDTextView.setText(
                                (rangingResult.getDistanceStdDevMm() / 1000f) + "");    //距離の偏差
                        addStandardDeviationOfDistanceToHistory(
                                rangingResult.getDistanceStdDevMm());
                        mRangeSDMeanTextView.setText(
                                (getStandardDeviationOfDistanceMean() / 1000f) + "");   //mean同上

                        mRssiTextView.setText(rangingResult.getRssi() + "");    //APのRSSI値を取得
            ...
            */
            ...
            sub.putExtra("range", String.valueOf(AP_Range));
            sub.putExtra("name", mScanResult.SSID);
            sub.putExtra("RSSI", mRssiTextView.getText().toString());
